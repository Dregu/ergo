...
static const struct zwlr_layer_surface_v1_listener zwlr_layer_surface_v1_listener = {.configure = zwlr_layer_surface_v1_configure};
...
void wayland_init(struct state* state)
{
    ...
    state->zwlr_layer_surface_v1 = zwlr_layer_shell_v1_get_layer_surface(
        state->zwlr_layer_shell_v1, state->wl_surface, state->output ? state->wl_output : NULL, ZWLR_LAYER_SHELL_V1_LAYER_TOP, "ergo");
...
struct wl_buffer* create_buffer(struct state* state)
{
    ...
    struct wl_buffer* buffer = wl_shm_pool_create_buffer(pool, 0, state->width, state->height, stride, WL_SHM_FORMAT_ARGB8888);
